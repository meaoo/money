...
class CategoryController extends AuthController {
    public function index(){
       $where['status']=0;
       $count=M('categories')->where($where)->count();
       $page = new \Think\Page($count, 9);
       $data=M('categories')->where($where)->order('id desc')->limit($page->firstRow.','.$page->listRows)->select();
       ...
       if(IS_POST && IS_AJAX){
         ...
         if(empty($category)){
           $this->ajaxReturn(1);
             ...
             $data['name']=$category;
             $data['status']=0;
             $info=M('categories')->where($data)->find();
             if($info){
                 $this->ajaxReturn(3);
             }else{
    ...
    public function editcat(){
        ...
        if($id){
            $where['id']=$id;
            $where['status']=0;
            $data=M('categories')->where($where)->find();
            if($data){
                $this->assign('data',$data);
            }
        }
